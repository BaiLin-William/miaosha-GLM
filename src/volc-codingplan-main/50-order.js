...
async function __volc_codingplan_probeCode(product) {
  ...
  if (__volc_codingplan_codeOverrides[cacheKey]) return __volc_codingplan_codeOverrides[cacheKey];
  ...
  __volc_codingplan_allIndexKeys.forEach(function(k) { if (k && ikSet.indexOf(k) === -1) ikSet.push(k); });

  console.log('[volc-codingplan-main] probe ' + __volc_codingplan_billingLabel(duration) + ' [code=' + baseCode + ' x keys=' + ikSet.length + ']...');
  ...
  for (let i = 0; i < ikSet.length; i++) {
    ...
    if (r.errCode === 'TransferError') {
      ...
      __volc_codingplan_codeOverrides[cacheKey] = ov;
      console.log('[volc-codingplan-main] v ' + __volc_codingplan_billingLabel(duration) + ' valid (code + key=' + ik + ') [out of stock = config accepted]');
      return ov;
    }
    if (r.orderId) {
      const ov = { IndexKey: ik, ConfigurationCode: baseCode, ChargeItemCode: origCharge };
      __volc_codingplan_codeOverrides[cacheKey] = ov;
      console.log('[volc-codingplan-main] ! ' + __volc_codingplan_billingLabel(duration) + ' stock found during probe, order ignored (orderId=' + r.orderId + ')');
      return ov;
    }
  }
  console.warn('[volc-codingplan-main] x no valid IndexKey found for ' + __volc_codingplan_billingLabel(duration));
  return null;
}
...
async function __volc_codingplan_tryOrder(product, indexKey) {
  ...
  const webId = cookies['monitor_huoshan_web_id'];
  if (!csrf || !webId) {
    __volc_codingplan_setStatus('not logged in');
    __volc_codingplan_stopRefresh('not logged in');
  ...
  const configBody = __volc_codingplan_normalizeConfigBody(product.raw);
  const body = JSON.stringify({ IndexKey: indexKey, ConfigList: [configBody], SignPay: true });

  __volc_codingplan_setStatus(tag + 'ordering...');
  let res;
  try {
    ...
    __volc_codingplan_setStatus(tag + 'network error');
    return { ok: false, retryable: false };
  }
  const data = await res.json();
  ...
  if (error) {
    ...
    if (isStockError) { __volc_codingplan_setStatus(tag + 'out of stock'); }
    else if (isConfigError) { __volc_codingplan_setStatus(tag + 'config invalid [' + detail + ']'); }
    else { __volc_codingplan_setStatus(tag + 'unavailable [' + detail + ']'); }
    return { ok: false, retryable: isConfigError, error: error };
  }
  const orderId = data.Result?.CustomerOrderID;
  if (!orderId) {
    __volc_codingplan_setStatus(tag + 'no order returned');
    return { ok: false, retryable: false };
  }
  __volc_codingplan_stopRefresh('order ' + orderId + ' created');
  ...
  __volc_codingplan_setStatus('order ' + orderId + ' created, redirecting...');
  ...
  __volc_codingplan_postCmd('VOLC_PURCHASE_SUCCESS', {
    ...
    plan: __volc_codingplan_config.title, payUrl: payUrl,
  });
...
async function __volc_codingplan_createOrder(productId) {
  ...
  if (!product || !product.raw) {
    __volc_codingplan_setStatus('config not found: ' + productId);
    return false;
  }
  const raw = product.raw;
  const ovKey = (raw.configBody.ConfigurationCode || '') + '|' + (raw.configBody.Duration || 1);
  const ov = __volc_codingplan_codeOverrides[ovKey];
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    if (i < candidates.length - 1) {
      __volc_codingplan_setStatus(__volc_codingplan_formatProductTag(product) + 'config invalid, trying next key...');
    }
  }
  return false;
}
...
window.addEventListener('message', function(event) {
  ...
  if (data.type === 'VOLC_AUTH_STATUS') {
    __volc_codingplan_setStatus(data.data?.ok ? 'ready' : 'not logged in');
  }
  if (data.type === 'VOLC_ORDER_RESULT') {
    if (data.error) { __volc_codingplan_setStatus('order failed: ' + data.error); }
    else if (data.data) { __volc_codingplan_setStatus('order ' + data.data.orderId + ' created'); }
  }
});
